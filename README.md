...
- **20 real bugs found & fixed** — build-tooling incompatibilities (JDK 25 vs. Gradle, Lombok, Mockito, Spring Boot's bundled ASM), two real security fixes in the login flow (a user-enumeration issue and its timing-side-channel sibling), a live IDOR the code-review audit caught and closed, a Kafka poison-pill gap, a second cross-service log-injection trace, a data-integrity validation gap, a payment-decline path that never existed, a refund compensation that never checked whose order it was refunding, an inventory module that could never actually allocate anything in a real run, a missing DB constraint that let duplicate Kafka delivery create two tickets for one order, and a Gradle task-ordering bug that could silently break the consolidated test report — found and fixed across `order-service`'s, `payment-service`'s, and `restaurant-service`'s code-review passes
