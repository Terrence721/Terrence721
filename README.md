...
- **Frontend in progress, TypeScript from file one**: all 16 planned API service modules, both context files (`AuthContext`, `FeedContext`), and all 31 planned components done; route pages and the app entry point are what's left. 36 more real bugs found along the way; a real gap disclosed too — 50 of the frontend files built so far have no behavioral tests yet, tracked openly rather than hidden
