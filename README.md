...
- **Genuinely early**: 2 of 40 workspace packages have real source started — `directus` (the CLI wrapper, complete) and `types` (56 of ~56 planned files — every real source file written, only the closing `websockets/index.ts` barrel left) — the rest are migrated `package.json` manifests, with source trees still to come one at a time
