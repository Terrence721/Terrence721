...
- **Genuinely early**: 2 of 42 workspace packages have real source started — `directus` (the CLI wrapper, complete) and `types` (partial, 37 of ~54 planned files) — the rest are migrated `package.json` manifests, with source trees still to come one at a time
