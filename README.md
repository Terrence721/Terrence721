...
- **Genuinely early**: 4 of 40 workspace packages have real source — `directus` (the CLI wrapper, complete but not runnable yet), `types` (source complete, 56 of 56 files, though it doesn't fully type-check yet — 2 cross-package dependencies still manifest-only, down from 3 once `schema` completed and closed its half of the gap), `schema` (source complete, 15 of 15 files, builds for real), and `constants` (newly started, 4 of 12 files — the second of `types`'s two remaining blockers) — the rest are migrated `package.json` manifests, with source trees still to come one at a time
