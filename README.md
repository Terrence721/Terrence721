...
- **31 real bugs found & fixed** — build-tooling incompatibilities (JDK 25 vs. Gradle, Lombok, Mockito, Spring Boot's bundled ASM), two real security fixes in the login flow (a user-enumeration issue and its timing-side-channel sibling), a live IDOR the code-review audit caught and closed, a Kafka poison-pill gap, a second cross-service log-injection trace, a data-integrity validation gap, a payment-decline path that never existed, a refund compensation that never checked whose order it was refunding, an inventory module that could never actually allocate anything in a real run, a missing DB constraint that let duplicate Kafka delivery create two tickets for one order, a Gradle task-ordering bug that could silently break the consolidated test report, a 10-day-old CodeQL alert finally run to ground and resolved (genuinely fixed, not just re-flagged), the same unbounded-Kafka-timeout and missing-error-handler gaps already found once each, caught a third time, an unlocked stock-deduction race reachable once `restaurant-service` scales past one replica, a login call silently executing on a shared Reactor Netty event-loop thread instead of an isolated scheduler, two CWE-117 log-injection bugs on the gateway's auth path (a malformed JWT's decoded payload, and the login email itself) confirmed by reading auth0's actual `java-jwt` source rather than assumed, a gateway circuit-breaker timeout that only surfaced by actually running the full containerized stack, an SSE stream that replayed stale buffered status updates to a late-connecting client (a Reactor `Sinks` semantics gap), a Spring Cloud Gateway route-method predicate with zero preflight-awareness that silently dropped every real browser CORS preflight, and a second, subtler CORS bug rooted in a `HandlerMapping` race between the gateway's own routing and a local `@RestController` — found and fixed across `order-service`'s, `payment-service`'s, `restaurant-service`'s, and `api-gateway-service`'s code-review passes, the container rollout, and the frontend build itself
...
An original, from-scratch event-driven platform simulating a utility company's meter-reading, usage-aggregation, and billing pipeline — not a fork or a cloned reference app. Designed to exercise the same skills a Principal Developer role expects: microservices decomposition, event streaming, polyglot service boundaries, resilient service-to-service communication, CI/CD, and production-grade observability, with the full architecture, event contracts, and rationale written up before a line of service code was. **Phases 1 through 3 are all done**: a direct-REST core loop first, then real Kafka events through a real Confluent Schema Registry, then a genuine Account/Customer Service and a polyglot Node.js/TypeScript Notification Service — five services verified live end-to-end, including a real `AccountId`-keyed invoice landing in Postgres and a real webhook actually received, backed by 53 automated tests.

- Deliberately staged across 6 build phases: a direct-REST core loop (Meter Simulator → Usage Aggregation → Billing) on a single Postgres database first, then Kafka + a schema registry, then Account/Customer Service + a polyglot Node.js Notification Service — all three now complete; a React/Redux Toolkit dashboard behind a BFF gateway, GitHub Actions CI/CD, and OpenTelemetry observability come next
- .NET Aspire orchestrating every service from day one; rate-plan billing logic built around the Strategy pattern (flat, tiered, time-of-use), documented in the [design document](https://github.com/Terrence721/GridPulse/blob/main/docs/gridpulse-design-doc.html)
- 18 real bugs found and fixed by actually running the thing, not just compiling it — logged in [`todo.md`](https://github.com/Terrence721/GridPulse/blob/main/todo.md)

Phases 1 through 3 all complete — see the [README](https://github.com/Terrence721/GridPulse#-build-phases) for current phase status rather than assuming it's further along.
