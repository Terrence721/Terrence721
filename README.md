...
- **Genuinely early**: 2 of 40 workspace packages have real source — `directus` (the CLI wrapper, complete but not runnable yet) and `types` (source complete, 56 of 56 files, though it doesn't fully type-check yet — 3 cross-package dependencies still manifest-only) — the rest are migrated `package.json` manifests, with source trees still to come one at a time
