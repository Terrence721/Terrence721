...
- **1,578 Pest tests**, real pre-existing bugs found and fixed along the way (documented, not hidden)
...
- **5,407 Vitest tests, 0 lint errors**, across all 13 modules added
...
- **6/6 backend services complete, 135/135 tests passing**, verified against real Postgres and Kafka infrastructure, not mocked
- **25 real bugs found & fixed** — build-tooling incompatibilities (JDK 25 vs. Gradle, Lombok, Mockito, Spring Boot's bundled ASM), two real security fixes in the login flow (a user-enumeration issue and its timing-side-channel sibling), a live IDOR the code-review audit caught and closed, a Kafka poison-pill gap, a second cross-service log-injection trace, a data-integrity validation gap, a payment-decline path that never existed, a refund compensation that never checked whose order it was refunding, an inventory module that could never actually allocate anything in a real run, a missing DB constraint that let duplicate Kafka delivery create two tickets for one order, a Gradle task-ordering bug that could silently break the consolidated test report, a 10-day-old CodeQL alert finally run to ground and resolved (genuinely fixed, not just re-flagged), the same unbounded-Kafka-timeout and missing-error-handler gaps already found once each, caught a third time, an unlocked stock-deduction race reachable once `restaurant-service` scales past one replica, and a login call silently executing on a shared Reactor Netty event-loop thread instead of an isolated scheduler, verified empirically against a real running server — found and fixed across `order-service`'s, `payment-service`'s, `restaurant-service`'s, and `api-gateway-service`'s code-review passes
...
The code-review audit itself is still in progress — `user-contract`, `user-service`, `order-service`, `payment-service`, and `restaurant-service` are closed out, `api-gateway-service` is now the last module in progress — see `todo.md` for current status rather than assuming it's finished.
...
- **Frontend complete, TypeScript from file one**: all 16 planned API service modules, both context files, all 31 components, all 12 route pages, and the app entry point are done — the full stack runs end to end, verified live against a real Postgres database. 36 more real bugs found along the way; a real gap disclosed too — 65 of the frontend files built so far have no behavioral tests yet, tracked openly rather than hidden
...
- **5 of 21 projects done** (EventBus, EventBusRabbitMQ, eShop.ServiceDefaults, IntegrationEventLogEF, Identity.API/Duende IdentityServer) — real bugs found and fixed along the way, not just version bumps: a Polly retry pipeline that never actually awaited its own operation (so it silently never retried the failures it was configured to catch), a null-conditional that made an error-handling branch unreachable dead code, a disabled JWT audience check that would have let a token issued for one downstream API be replayed against another, a silent event-type-collision bug in the transactional outbox's reflection-based type resolver, and a CodeQL-caught log-injection spot plus an external-login callback that crashed instead of falling back cleanly, both found in Identity.API's own Duende-shipped source
- **Every completed project is fully tested**: MSTest on .NET's newer Microsoft.Testing.Platform runner, 174 passing tests across all 5 done projects, with CI coverage collection and PR-visible test reporting verified against real GitHub Actions runs. That Polly retry fix above? Verified end-to-end for the first time by one of those tests — not just fixed and assumed correct
...
- **A real SQL injection found and fixed while porting `packages/schema`**: `sqlite.ts`'s `hasColumn()` built its query via direct string interpolation of table/column names instead of parameterized binding, unlike every other method in the same file — verified exploitable and fixed against a real in-memory SQLite database, not assumed
- **A real circular import found and surgically removed while porting `packages/schema-builder`**: upstream's `collection.ts`/`field.ts`/`relation.ts` mutually import each other as runtime values — verified not a bug (every cross-class instantiation is deferred inside a method body, so ESM's live-binding resolution handles it safely), then still extracted the one piece of logic actually causing it into a new `SchemaBuilder`-owned helper, breaking the cycle into a clean acyclic chain. Verified against upstream's own real test suite: 15/15 passing, identical output
- **Genuinely early**: 8 of 40 workspace packages have real source — `directus` (the CLI wrapper, complete but not runnable yet), `types` (source complete, 57 of 57 files, and now fully type-checks and builds for real — all 3 original cross-package dependencies resolved once `schema`, `constants`, and `ai` each completed), `schema` (source complete, 15 of 15 files, builds for real), `constants` (source complete, 11 of 11 files, builds for real), `ai` (source complete, 4 of 4 files, builds for real — the last of `types`'s three original blockers), `schema-builder` (source complete, 6 of 6 files, builds for real — one of the three blockers of `utils`, the package with the widest fan-in of anything left), `errors` (source complete, 69 of 69 files, builds for real — the second blocker of `utils` — three real, disclosed deviations found while porting it: a missing dedicated error code, an upstream type map that silently narrowed 7 codes to `never`, and a `.test-d.ts` file upstream never wired up to run), and `system-data` (in progress, 3 of 46 `src/` files — the last blocker of `utils`, a structurally new mixed `.ts`/`.yaml` pattern, already turned up its own real, disclosed type gap) — the rest are migrated `package.json` manifests, with source trees still to come one at a time
