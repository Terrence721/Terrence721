...
- **11 real bugs found & fixed** — build-tooling incompatibilities (JDK 25 vs. Gradle, Lombok, Mockito, Spring Boot's bundled ASM), two real security fixes in the login flow (a user-enumeration issue and its timing-side-channel sibling), a live IDOR the code-review audit caught and closed, and a Kafka poison-pill gap fixed in the same pass
...
- **Genuinely early**: 2 of 42 workspace packages have real source started — `directus` (the CLI wrapper, complete) and `types` (partial, 42 of ~54 planned files) — the rest are migrated `package.json` manifests, with source trees still to come one at a time
